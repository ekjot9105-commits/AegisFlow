...
class APIResponse(BaseModel, Generic[T]):
    ...
    metadata: Optional[Dict[str, Any]] = Field(
        default_factory=dict, description="Additional context or pagination info"
    )
...
class ActionRecommendation(BaseModel):
    ...
    assigned_role: str = Field(
        ..., description="Role responsible for execution (e.g., Volunteer, Operator)"
    )
...
class VolunteerTask(BaseModel):
    ...
    location: str = Field(
        ..., description="Where the task should be performed (e.g., Gate 4)"
    )
    instructions: str = Field(
        ..., description="Step-by-step instructions for the volunteer"
    )
...
class CopilotRecommendation(BaseModel):
    ...
    reasoning_chain: List[str] = Field(
        ..., description="Step-by-step logical reasoning path taken by the AI"
    )
    ...
    situation_summary: str = Field(
        ..., description="Detailed summary of the current situation"
    )
    estimated_crowd_reduction: int = Field(
        ...,
        description="Estimated percentage reduction in crowd density if actions are taken",
    )
    recommended_actions: List[ActionRecommendation] = Field(
        ..., description="List of mitigation steps"
    )
    alternative_actions: List[ActionRecommendation] = Field(
        default_factory=list,
        description="Alternative mitigation steps with lower confidence or different trade-offs",
    )
    ...
    incident_summary: str = Field(
        ..., description="Brief summary for executive dashboard"
    )
...
class SensorTelemetry(BaseModel):
    ...
    metric_type: str = Field(
        ..., description="Type of data (e.g., crowd_density, temperature)"
    )
